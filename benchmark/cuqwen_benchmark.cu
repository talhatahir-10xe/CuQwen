...
int main() {
    ...
    std::cout << "[*] Starting Raw Inference Benchmark for " << TOTAL_TOKENS << " tokens..." << std::endl;

    // CUDA Events for fine-grained timer tracking
    ...
    // Warmup forward pass
    qwen_forward(dummy_tokens[0], 0, config, state, weights);
    ...
    for (int slice_start = 0; slice_start < TOTAL_TOKENS; slice_start += INTERVAL_STEP) {
        int slice_end = std::min(slice_start + INTERVAL_STEP, TOTAL_TOKENS);
        int current_step_count = slice_end - slice_start;

        if (current_step_count <= 0) break;
        ...
        double tok_sec = (current_step_count / (elapsed_ms / 1000.0));
    ...
    std::cout << "  • Initial Speed            : " << std::fixed << std::setprecision(2) << initial_speed << " tok/s\n";
    std::cout << "  • Final Speed              : " << std::fixed << std::setprecision(2) << final_speed << " tok/s\n";
