...
int qwen_forward(
    ...
    for (int l = 0; l < config.n_layers; ++l) {
        ...
        launch_fused_attn_block(
            ...
            pos, config.rope_theta, config.norm_eps,
            stream
        );
        ...
        for (int h = 0; h < n_kv; ++h) {
            half* k_dst = state.key_cache + layer_kv_offset + (size_t)h * config.max_seq_len * head_dim + pos * head_dim;
            half* v_dst = state.value_cache + layer_kv_offset + (size_t)h * config.max_seq_len * head_dim + pos * head_dim;

            const half* k_src = state.k + h * head_dim;
            const half* v_src = state.v + h * head_dim;

            CUDA_CHECK(cudaMemcpyAsync(k_dst, k_src, head_dim * sizeof(half), cudaMemcpyDeviceToDevice, stream));
            CUDA_CHECK(cudaMemcpyAsync(v_dst, v_src, head_dim * sizeof(half), cudaMemcpyDeviceToDevice, stream));
        }

        launch_gqa_attention(
            state.q, state.key_cache, state.value_cache, state.xb, state.att_scores,
            pos, config.n_heads, config.n_kv_heads, head_dim, layer_kv_offset, config.max_seq_len, stream
        );
    ...
    if (h_history != nullptr && history_len > 0) {
        CUDA_CHECK(cudaMemcpyAsync(state.d_history, h_history, history_len * sizeof(int), cudaMemcpyDeviceToDevice, stream));
        launch_apply_repetition_penalty(state.logits, state.d_history, history_len, config.vocab_size, config.repetition_penalty, stream);
    }
