...
int qwen_forward(
    ...
    for (int l = 0; l < config.n_layers; ++l) {
        ...
        // Async contiguous write per KV head into sequence slot `pos`
        for (int h = 0; h < n_kv; ++h) {
            half* k_dst = layer_k_cache + (size_t)h * config.max_seq_len * head_dim + pos * head_dim;
            half* v_dst = layer_v_cache + (size_t)h * config.max_seq_len * head_dim + pos * head_dim;
        ...
        launch_gqa_attention_decode(
            ...
            pos + 1, config.n_heads, config.n_kv_heads, head_dim,
