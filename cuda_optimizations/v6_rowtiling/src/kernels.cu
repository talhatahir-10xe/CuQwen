...
void launch_gqa_attention_decode(
    ...
    int total_len, int n_heads, int n_kv_heads, int head_dim,
    int max_seq_len, float* d_partial_out, float* d_partial_max,
    float* d_partial_sum, cudaStream_t stream
) {
    float scale = 1.0f / sqrtf(static_cast<float>(head_dim));
    int splits_to_launch = (total_len + SPLIT_K_SIZE - 1) / SPLIT_K_SIZE;

    dim3 grid_stage1(n_heads, splits_to_launch);
...
__global__ void fused_mlp_stage1_kernel(
    ...
    for (int c = tid; c < vec_dim; c += blockDim.x) {
        ...
        #pragma unroll
        for (int k = 0; k < 4; ++k) {
            // xn = x * inv_rms * norm_weight (computed in registers, no smem)
...
void launch_fused_mlp_stage2(
    ...
    int blocks  = (dim + MLP2_TILE_ROWS - 1) / MLP2_TILE_ROWS;  // 384 instead of 1536
    int threads = 256;
...
__global__ void compute_logits_kernel(
    ...
    // Main dot product loop: x_normed read once (from L2), dotted against 4 weight rows
    #pragma unroll 4
    ...
    // Cross-warp via static smem (512 bytes — no occupancy cost)
